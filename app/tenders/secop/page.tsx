...
type Profile = {
  ...
  departments: string[]; min_value: number; max_value: number; minimum_days: number; last_synced_at: string | null;
};
type Opportunity = {
...
export default async function SecopRadarPage({ searchParams }: { searchParams: Promise<{ company?: string; decision?: string }> }) {
  ...
  const [storedProfile] = companyId ? await dbSelect<Profile>("secop_profiles", {
    select: "company_id,keywords,excluded_keywords,unspsc_codes,departments,min_value,max_value,minimum_days,last_synced_at",
  ...
  const opportunities = companyId ? await dbSelect<Opportunity>("secop_opportunities", {
    select: "id,reference,title,description,entity,department,city,modality,base_price,submission_date,unspsc_code,source_url,match_score,match_reasons,decision,tender_id",
    company_id: `eq.${companyId}`,
  ...
  return <AppShell>
    <div className="flex flex-wrap items-start justify-between gap-5">
      ...
      <div className="flex gap-3"><Link href="/tenders" className="rounded-xl border px-4 py-3 font-semibold">Licitaciones</Link>{companyId && <form action={syncSecop}><input type="hidden" name="company_id" value={companyId} /><button className={primary}>Sincronizar ahora</button></form>}</div>
    </div>
