...
export default async function NewProjectPage(){
  ...
  const uniqueCompanies=companies.filter((company,index,all)=>all.findIndex(item=>(item.name??"").trim().toLocaleLowerCase("es")===(company.name??"").trim().toLocaleLowerCase("es"))===index);
  const uniqueBranches=branches.filter((branch,index,all)=>all.findIndex(item=>(item.name??"").trim().toLocaleLowerCase("es")===(branch.name??"").trim().toLocaleLowerCase("es"))===index);
  const uniqueClients=clients.filter((client,index,all)=>all.findIndex(item=>(item.name??"").trim().toLocaleLowerCase("es")===(client.name??"").trim().toLocaleLowerCase("es"))===index);
  ...
  <form action={createProject} className="mt-8 grid gap-6 rounded-2xl border bg-white p-7 shadow-sm md:grid-cols-2">
    ...
    <Field label="Empresa"><Select name="company_id" options={uniqueCompanies} required/></Field><Field label="Sede"><Select name="branch_id" options={uniqueBranches}/></Field><Field label="Cliente"><Select name="client_id" options={uniqueClients}/>{!uniqueClients.length&&<Link href="/commercial/clients/new" className="mt-2 inline-block text-sm font-semibold text-orange-600">Crear el primer cliente →</Link>}</Field><Field label="Director"><Select name="manager_id" options={profiles}/></Field>
