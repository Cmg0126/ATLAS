...
export default async function OpportunityDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <AppShell>
    ...
    <div className="mt-7 grid gap-6 xl:grid-cols-2">
      ...
      <section className="rounded-2xl border bg-white p-6 shadow-sm"><h2 className="text-xl font-bold">Nueva cotización</h2><p className="mt-1 text-sm text-zinc-500">El total se calcula con las partidas.</p><form action={createQuotation} className="mt-5 grid gap-4 md:grid-cols-2"><input type="hidden" name="opportunity_id" value={id} /><Field label="Número"><input name="quotation_number" required className={input} placeholder="COT-2026-001" /></Field><Field label="Válida hasta"><input type="date" name="validity_date" className={input} /></Field><Field label="Estado"><select name="status" className={input}><option value="DRAFT">Borrador</option><option value="SENT">Enviada</option><option value="APPROVED">Aprobada</option><option value="REJECTED">Rechazada</option></select></Field><Field label="Notas"><input name="notes" className={input} placeholder="Condiciones comerciales" /></Field><button className="rounded-xl bg-orange-500 px-5 py-3 font-semibold text-white md:col-span-2">Crear cotización</button></form></section>
    </div>
    <QuotationManager opportunityId={id} opportunityTitle={opportunity.title} quotations={quotations} companies={companies} branches={branches} />
  </AppShell>;
}
